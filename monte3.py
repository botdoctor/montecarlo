...
def read_input_file(filename):
    ...
    max_age = end_year - (CURRENT_YEAR - 10)  # Max age based on oldest song by end_year
    ...
    # Calculate max_age for multipliers
    min_release_year = CURRENT_YEAR - max_initial_age
    max_age = end_year - min_release_year
    ...
    return iterations, end_year, mean_yoy_by_age, std_yoy_by_age, growth_multipliers, song_age_groups

def monte_carlo_simulation(initial_streams, release_year, mean_yoy_by_age, std_yoy_by_age, growth_multipliers, years, trials):
    """
    Run Monte Carlo simulation with age-dependent YOY growth, accumulating streams by year.
    ...
    for i in range(trials):
        ...
        for year_offset in range(years + 1):
            ...
            mean_yoy = mean_yoy_by_age.get(song_age, 0.05)  # Default if age missing
...
def main():
    ...
    try:
        iterations, end_year, mean_yoy_by_age, std_yoy_by_age, growth_multipliers, song_age_groups = read_input_file(input_file)
    ...
    for song_age, initial_streams in song_age_groups.items():
        ...
        results = monte_carlo_simulation(
            initial_streams, release_year, mean_yoy_by_age, std_yoy_by_age, growth_multipliers, years, trials=iterations
        )
